...
(`qbit_core.features.torrents.TorrentFilter`/`select_torrents`): each
...
def test_list_unfiltered_zero_torrents_is_not_an_error(
    ...
    """There was nothing to filter out, so it is a legitimate empty result, not
    NO_MATCH.
    """
...
def test_list_json_and_jsonl_have_the_same_filters_schema(
    ...
    """JSONL is only a compact one-line serialization of the same payload, never
    a schema of its own.
    """
    configure_qbit_backend(client=_client_with_categories_and_states())
...
def test_mutation_missing_selector_is_refused(
    ...
    """A bulk mutation with no --hash, --all, or filter must never silently mean
    the whole seedbox.
    """
    client = FakeQbitClient(torrents=[make_torrent(hash=TORRENT_A)])

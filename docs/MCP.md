...
It is not the qbit-ops CLI over MCP. The CLI has 39 filter options on
`torrents list` alone; this has four tools, and exposes a fraction of
what they can do. That is the design, not a gap to close.

The reason is measured: `torrents list --format json` over a
1000-torrent library serialises around 346 KiB. An MCP tool result
cannot be piped through `jq` or `head` -- it lands in the model's
context whole. So the surface is built around drill-down instead:

    library_summary  ->  find_torrents  ->  inspect_torrent
                                        ->  explain_torrent

Each step returns enough to choose the next one. On the same 1000-torrent
library, the worst case an agent can ask for is about 7.8 KiB.
...
The MCP SDK is an optional extra, so a normal install never needs it:
